...
# vyogo.tech/frappe-operator-bundle:$VERSION and vyogo.tech/frappe-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= vyogo.tech/frappe-operator
...
ifneq ($(REPLACES),)
BUNDLE_GEN_FLAGS += --replaces $(REPLACES)
endif
...
sync-helm-crds: ## Copy CRDs from config/crd/bases to helm/frappe-operator/crds/ so the Helm chart stays in sync.
	@cp config/crd/bases/vyogo.tech_*.yaml helm/frappe-operator/crds/
