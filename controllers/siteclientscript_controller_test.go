...
func TestSiteClientScriptReconciler_Reconcile(t *testing.T) {
	...
	clientScript := &vyogotechv1.SiteClientScript{
		...
		Spec: vyogotechv1.SiteClientScriptSpec{
			SiteRef: &vyogotechv1.NamespacedName{
				Name: "test-site",
			},
			DT:      "Customer",
			Script:  "frappe.ui.form.on('Customer', { refresh(frm) {} });",
			Enabled: true,
		},
	}
	...
	client := fake.NewClientBuilder().
		WithScheme(scheme).
		WithObjects(site, adminSecret, clientScript).
		WithStatusSubresource(clientScript).
		Build()

	frappeClient := NewFrappeClient(mockServer.URL, "Administrator", "adminpass")
	frappeClient.SID = "mock-sid"

	r := &SiteClientScriptReconciler{
		Client:       client,
		Scheme:       scheme,
	...
	updated := &vyogotechv1.SiteClientScript{}
	if err := client.Get(context.Background(), req.NamespacedName, updated); err != nil {
		t.Fatalf("failed to fetch updated ClientScript: %v", err)
	}
