...
func TestSiteUserPermissionReconciler_Reconcile(t *testing.T) {
	...
	userPerm := &vyogotechv1.SiteUserPermission{
		...
		Spec: vyogotechv1.SiteUserPermissionSpec{
	...
	client := fake.NewClientBuilder().
		WithScheme(scheme).
		WithObjects(site, adminSecret, userPerm).
		WithStatusSubresource(userPerm).
		Build()

	frappeClient := NewFrappeClient(mockServer.URL, "Administrator", "adminpass")
	frappeClient.SID = "mock-sid"

	r := &SiteUserPermissionReconciler{
		Client:       client,
		Scheme:       scheme,
	...
	updated := &vyogotechv1.SiteUserPermission{}
	if err := client.Get(context.Background(), req.NamespacedName, updated); err != nil {
		t.Fatalf("failed to fetch updated UserPermission: %v", err)
	}
