...
func TestSitePropertySetterReconciler_Reconcile(t *testing.T) {
	...
	propSetter := &vyogotechv1.SitePropertySetter{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-prop-setter",
			Namespace: "default",
		},
		Spec: vyogotechv1.SitePropertySetterSpec{
	...
	client := fake.NewClientBuilder().
		WithScheme(scheme).
		WithObjects(site, adminSecret, propSetter).
		WithStatusSubresource(propSetter).
		Build()

	frappeClient := NewFrappeClient(mockServer.URL, "Administrator", "adminpass")
	frappeClient.SID = "mock-sid"

	r := &SitePropertySetterReconciler{
		Client:       client,
		Scheme:       scheme,
	...
	updated := &vyogotechv1.SitePropertySetter{}
	if err := client.Get(context.Background(), req.NamespacedName, updated); err != nil {
		t.Fatalf("failed to fetch updated PropertySetter: %v", err)
	}
