...
func (r *FrappeBenchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, bench); err != nil {
		...
		ReconciliationErrors.WithLabelValues("frappebench", "fetch_error").Inc()
	...
	if err := r.updateBenchStatus(ctx, bench, gitEnabled, fpmRepos); err != nil {
		...
		ReconciliationErrors.WithLabelValues("frappebench", "status_update_error").Inc()
		ReconciliationDuration.WithLabelValues("frappebench", "error").Observe(time.Since(startTime).Seconds())
		return ctrl.Result{}, err
	}

	// Track resource count
	ResourceTotal.WithLabelValues("frappebench", bench.Namespace).Inc()

	// Record successful reconciliation duration
	ReconciliationDuration.WithLabelValues("frappebench", "success").Observe(time.Since(startTime).Seconds())
